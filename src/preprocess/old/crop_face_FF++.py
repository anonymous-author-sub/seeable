        ...
        try:
            ...
            if boxes[largest].shape[0] > 1:
                ...
                for i, iboxes in enumerate(boxes[largest]):
                    x, y, size = get_boundingbox(iboxes.flatten(), width, height)
                    cropped_face = vframe[y : y + size, x : x + size]
                    # save all
                    # cv2.imwrite(os.path.join(save_root_face,"%s-face_%d-prob_%f.png") % (s,i,probs[i]), cropped_face)

                    cropped_mask = mframe[y : y + size, x : x + size]
                    # cv2.imwrite(('mask-%s.png')%s,mframe)
                    # cv2.imshow('cropped_face',cropped_face)
                    # cv2.imshow('cropped_mask',cropped_mask)
                    # cv2.waitKey(0)

                    if (
                        cropped_mask.sum() == mframe.sum()
                    ):  # if whole mask is in our face save face and update boxes and points
                        # if save_root:
...
if __name__ == "__main__":
    ...
    BASE_DIR = "/media/borutb/disk11/DataBase/DeepFake/FaceForensics++/"  # The base dir of DeepFake dataset
    ...
    for df in ["original_sequences"]:
        ...
        for line in tqdm(data):  # data[:100] only subset
            ...
            maskpath[-3] = "masks"
            maskpath = os.path.join("/", *maskpath)
            # if masks not exist take Deepfakes masks
            if not os.path.exists(maskpath):
                maskpath = maskpath.split("/")
                maskpath[-4] = "Deepfakes"
                if maskpath[-5] == "original_sequences":
                    maskpath[-5] = "manipulated_sequences"
                name = maskpath[-1].split(".")[0]
                maskpath = os.path.join("/", *maskpath)
                mp4file = [
                    f for f in listdir(os.path.dirname(maskpath)) if f.startswith(name)
                ]
                maskpath = maskpath.split("/")
                maskpath[-1] = mp4file[0]
                maskpath = os.path.join("/", *maskpath)
