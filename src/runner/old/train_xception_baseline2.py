...
from network.utils import DatasetCSV
import csv
...
def train_model(
    model, model_dir, criterion, optimizer, scheduler, num_epochs=10, current_epoch=0
):
    best_logloss = 10.0
    best_epoch = 0
        ...
        for phase in ["train", "test"]:
            ...
            for i, (inputs, labels) in enumerate(dataloaders[phase]):
                ...
                if (i + 1) % 500 == 0 and (phase == "train"):
                    ...
                    running_loss_train = 0.0
                    test_loss = val_models(
                        model, criterion, num_epochs, test_list, epoch
                    )
                    writer.add_scalars(
                        phase + "ing loss",
                        {"val": test_loss},
                    ...
                    log.write(
                        "save current model {}, Now time is {}, best logloss is {}\n".format(
                            i,
                    ...
                    model_out_paths = os.path.join(
                        model_dir, str(epoch) + "_" + str(i) + "_xception.ckpt"
                    )
                    torch.save(
                        model.module.state_dict(), model_out_paths
                    )  # shranimo vse
                    model.train()
                    # scheduler.step()
            ...
            if phase == "train":  # dodal BB
                scheduler.step()
            if phase == "test":
                epoch_loss = running_loss / len(test_list)
                writer.add_scalars(
                    "training loss",
                    {"val_epoch": epoch_loss},
                    (epoch + 1) * len(dataloaders["train"]),
                )
                log.write("i= {}\n".format((epoch + 1) * len(dataloaders["train"])))
    ...
    log.write("***************************************************")
    log.write(
        "Best logloss {:.4f} and Best Epoch is {}\n".format(best_logloss, best_epoch)
    )
...
def val_models(model, criterion, num_epochs, test_list, current_epoch=0, phase="test"):
    ...
    for k, (inputs_val, labels_val) in enumerate(dataloaders[phase]):
        ...
        running_loss_val2 += batch_loss * len(labels_val)
    ...
    epoch_loss2 = running_loss_val2 / len(test_list)
    y_trues, y_scores = np.array(y_trues), np.array(y_scores)
    accuracy = accuracy_score(y_trues, np.where(y_scores > 0.5, 1, 0))
...
if __name__ == "__main__":
    ...
    #  Output path
    model_dir = "/media/borutb/disk11/DataBase/DeepFake/Celeb-DF-v2/models/xception2/"
    ...
    log.write(
        "model : xception   batch_size : " + str(batch_size) + "   frames : I-frames \n"
    )
    # log.write('model : xception   batch_size : 32,   frames : I-frames \n')
    ...
    params = (
        {"shuffle": False, "num_workers": 4, "pin_memory": True} if use_cuda else {}
    )
    ...
    image_datasets = {}
    # over sampling
    image_datasets["train"] = data.DataLoader(
    ...
    writer = SummaryWriter("runs/xception_experiment_2")
