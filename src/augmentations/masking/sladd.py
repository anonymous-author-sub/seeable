...
class SladdMasking(Masking):
    ...
    def __init__(self, compose: bool = False, single: bool = True, **kwargs):
    ...
    def compute_mask(self, img: np.ndarray, landmarks, idx: int) -> np.ndarray:
        ...
        regs = self.regions[idx]
        # if isinstance(reg, int):
        #    mask = parse(img, reg, landmarks)
        masks = [SladdMasking.parse(img, reg, landmarks) for reg in regs]
